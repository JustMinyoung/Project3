package com.care.project3.common;
...
public class BoardTools {
	public static String getNavi(int currentPage, int pageBlock, int totalPage, String url) {
		...
		String result="";
		if(currentPage!=1)	result+="<a href='"+url+(currentPage-1)+"'> [PREV] </a>";
		for(int i=1;i<=blockCnt;i++) {
			if(currentPage==i)	result+="<b>";
			result+="<a href='"+url+i+"'> "+ i +" </a>";
			if(currentPage==i)	result+="</b>";
		}
		if(currentPage!=blockCnt) 	result+="<a href='"+url+(currentPage+1)+"'> [NEXT] </a>";
		return result;
	}
	...
	public static String getSearchWord(List<String> lst, String scriptFuncName){
		...
		result += " name='searchBtn' value='�˻�' style='width: 80px; '/>";
